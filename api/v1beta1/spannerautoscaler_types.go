...
type CPUWindowMetric struct {
	// The CPU metric the aggregates were computed from: `HighPriority` or `Total`.
	Metric CPUMetricType `json:"metric"`

	// The window the aggregates cover, exactly as declared in
	// spec.scaleConfig.metricWindows (e.g. "15m").
	Window string `json:"window"`

	// Minimum 1-minute CPU utilization (percent) within the window.
	Min int `json:"min"`

	// Average 1-minute CPU utilization (percent) within the window.
	Avg int `json:"avg"`

	// Maximum 1-minute CPU utilization (percent) within the window.
	Max int `json:"max"`
}
...
type SpannerAutoscalerStatus struct {
	...
	// +optional
	CurrentCPUWindowMetrics []CPUWindowMetric `json:"currentCPUWindowMetrics,omitempty"`
