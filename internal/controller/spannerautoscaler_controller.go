...
func (r *SpannerAutoscalerReconciler) needUpdateProcessingUnits(log logr.Logger, sa *spannerv1beta1.SpannerAutoscaler, desiredProcessingUnits int, now time.Time) bool {
	...
	case scaling.DecisionSkipScaleUpInterval:
	...
	case scaling.DecisionSkipScaleDownInterval:
	...
	case scaling.DecisionSkipScaleDownWindow:
...
// The logic lives in internal/scaling so the offline simulator shares it.
func calcDesiredProcessingUnits(sa spannerv1beta1.SpannerAutoscaler) int {
	return scaling.DesiredProcessingUnits(sa)
}
...
func calcDesiredPUFromCPU(currentCPU, targetCPU int, sa spannerv1beta1.SpannerAutoscaler) int {
	return scaling.DesiredPUFromCPU(currentCPU, targetCPU, sa)
}
...
// range and the currently active schedules. See scaling.DesiredPURange for
// the full semantics (Exceed/Cap policies, rounding, clamping).
func calcDesiredPURange(sa spannerv1beta1.SpannerAutoscaler) (minPU, maxPU int, changed, capped bool) {
	return scaling.DesiredPURange(sa)
}
