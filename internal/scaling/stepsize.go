...
func NextRampPU(current, target int, up, down RampStep, lastScaleTime, now time.Time) (int, time.Duration) {
	...
	interval := DurationOr(step.Interval, step.DefaultInterval)
	...
	if stepSize > 0 {
		if target > current {
			next = min(current+stepSize, target)
		} else {
			next = max(current-stepSize, target)
		}
	}
...
// logic that lived in the controller's calcDesiredProcessingUnits:
//
...
// fallback: 2000 for scaledown, 0 for scaleup. These match the historical
// inline fallbacks.
//
// This helper is shared by the CPU-driven path and the SpannerManualScaling
// path so that percentage / rounding semantics stay identical. The
// differential test in the controller's stepsize_test.go locks down
// equivalence with the legacy inline version against the parameter space the
// CPU path exercises.
