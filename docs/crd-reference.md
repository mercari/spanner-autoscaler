...
| `scaleUp` _[IntOrString](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.22/#intorstring-intstr-util)_ | How many processing units to add to the current processing units when the condition<br />holds. Either a fixed number of processing units (e.g. 3000) or a percentage of the<br />current processing units (e.g. "25%"; at most "100%", i.e. doubling). The result is<br />rounded to a valid processing-unit value and clamped to the min/max range; a<br />percentage never resolves beyond twice the current processing units, even when<br />rounding up would cross that line (e.g. "100%" at 600 processing units resolves<br />to 1000, not 2000). |  |  |
